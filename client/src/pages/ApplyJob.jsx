...
const ApplyJob = () => {
  // ✅ Fixed spelling: userApplications (not userApplicatins)
  ...
  const fetchJob = async () => {
    try {
      ...
      const { data } = await api.get(`/api/jobs/${id}`); // ✅ Using api

      if (data.success) {
        setJobData(data.job);
        console.log('✅ Job fetched:', data.job.title);
      } else {
        toast.error(data.message);
      }
    } catch (error) {
  ...
  const applyHandler = async () => {
    try {
      ...
      const { data } = await api.post(
        '/api/users/apply', // ✅ Using api with relative path
        { jobId: JobData._id },
        { headers: { Authorization: `Bearer ${token}` } }
      );
        ...
        <div className="bg-white text-black rounded-lg w-full">
          <div className="flex justify-center md:justify-between flex-wrap gap-8 px-14 py-20 mb-6 bg-sky-50 border border-sky-400 rounded-xl">
            ...
            <div className="flex flex-col justify-center text-end text-sm max-md:mx-auto max-md:text-center">
              <button
                onClick={applyHandler}
                className="bg-blue-600 px-10 py-3 rounded text-white hover:bg-blue-700 transition-colors"
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start">
            <div className="w-full lg:w-2/3">
              ...
              <button
                onClick={applyHandler}
                className="bg-blue-600 px-10 py-3 rounded text-white mt-10 hover:bg-blue-700 transition-colors"
                disabled={isAlreadyApplied}
              >
                {isAlreadyApplied ? "Already Applied" : "Apply Now"}
              </button>
